...
const works: Record<string, Work> = {
  ...
  sphere: {
    ...
    caseStudies: [
      {
        label: "方針",
        title: "Generative Artでバリエーションを担保する",
        body: "球面上に配置したパーティクルを多様な数学的手法で制御し、幾何学的な美しさを目指しました。Generative Artはパラメータを調整することで多様なGraphicが作れることが特徴です。ここにランダム要素を加えることで、周回のたびに新鮮なGraphicに出会えたり、眺め続けても時間と共に変化するグラフィックを実現しました。パーティクルシステム以外にもポストエフェクトもこの一助になっています。",
      },
      {
        label: "次への課題",
        title: "横割りより縦割り",
        body: "これまで、インフラ層、ドメイン層、UI層で担当するメンバーを分けて開発をしていました。この方法は各人の得意不得意を活かせるという点では良かったものの、「Domain都合でUIを実装した場合に遊びの体験が損なわれる」または「遊びの体験を重視した場合にDomainにまで変更が必要になる」といった上層と下層での不釣り合いが頻繁に生じました。これは今回DDDを採用したことで、UI層に一切のドメインロジックが許されなくなったことで顕著化しました。そこで今回の一番の反省としては上層から下層まで一貫して遊び手のためのデザインになるためには、横割り（層ごとの役割分担）ではなく縦割り（コンテンツごとの役割分担）が必要と結論づけました。",
      },
    ],
    tech: [
      {
  ...
  spiral: {
    ...
    caseStudies: [
      ...
      },
      {
        ...
        body: "ゲームをより美しくかつ直感的にするため、UIやGraphicの観点からゲームコンテンツの提案・意見を続けました。具体的には、螺旋の本数を増やすアップグレードの追加や、バトルシステムにおける状態の定義（準備・バトル・結果確認）などを提案しました。",
      },
    ],
    tech: [
      {
        ...
        items: [
          ...
          { name: "ラッパー層", detail: "UnityのUI要素を包む層。UI基盤の変更や差し替えに強くするため。" },
        ],
      },
      {
        tab: "Unity基盤",
...
const careerTimeline = [
  ...
  },
  {
    ...
    areas: ["グラフィック", "UI", "一部ゲームデザイン", "一部設計判断"],
  },
  {
    phase: "ゲーム開発 3年目",
    date: "2025.01 -",
